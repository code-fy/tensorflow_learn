...
class CSVInputV2(Input):
  ...
  def _build(self, mode, params):
    if self._input_path.startswith('hdfs://'):
      ...
      record_defaults = [ self.get_type_defaults(x, v) \
                         for x,v in zip(self._input_field_types,
                                        self._input_field_defaults) ]
      dataset = tf.data.experimental.make_csv_dataset([self._input_path],
                    self._data_config.batch_size, column_names=self._input_fields,
                    field_delim=self._data_config.separator,
                    column_defaults=record_defaults,
                    header=False, num_epochs=num_epochs,
                    shuffle=is_train,
                    num_parallel_reads=8, sloppy=is_train)
    ...
    dataset = dataset.map(map_func=self._preprocess,       

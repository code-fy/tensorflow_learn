...
class CSVInput(Input):
  ...
  def _parse_csv(self, line):
    ...
    with tf.control_dependencies([check_op]):
      fields = tf.decode_csv(line, field_delim=self._data_config.separator,
    ...
    inputs = { self._input_fields[x]: \
                _convert(fields[x], self._input_field_types[x],
  ...
  def _build(self, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      dataset = dataset.interleave(tf.data.TextLineDataset,
      ...
      dataset = dataset.shuffle(self._data_config.shuffle_buffer_size,
                        seed=2020, reshuffle_each_iteration=True)

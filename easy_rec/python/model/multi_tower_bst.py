...
class MultiTowerBST(CTRModel):
  def __init__(self, model_config, feature_configs, features,
    ...
    logging.info('all tower num: {0}'.format(self._tower_num + self._bst_tower_num))
  ...
  def dnn_net(self, net, dnn_units, name):
    with tf.variable_scope(name_or_scope=name, reuse=tf.AUTO_REUSE):
      for idx, units in enumerate(dnn_units):
        net = tf.layers.dense(net, units=units, activation=tf.nn.relu, name='%s_%d' % (name, idx))
    return net
  ...
  def attention_net(self, net, dim, cur_seq_len, seq_size, name):
    ...
    scores = tf.matmul(query_net, key_net, transpose_b=True)  #[B, seq_size, seq_size]

    hist_mask = tf.sequence_mask(cur_seq_len, maxlen=seq_size-1) #[B, seq_size-1]
    cur_id_mask = tf.ones([tf.shape(hist_mask)[0], 1], dtype=tf.bool) #[B, 1]
    mask = tf.concat([hist_mask, cur_id_mask], axis=1)  #[B, seq_size]
    masks = tf.reshape(tf.tile(mask, [1, seq_size]), (-1, seq_size, seq_size)) #[B, seq_size, seq_size]
  ...
  def multi_head_att_net(self, id_cols, head_count, emb_dim, seq_len, seq_size):
    ...
    multi_head_attention_res_net = self.dnn_net(multi_head_attention_res_net, [emb_dim], name='multi_head_attention')
  ...
  def bst(self, bst_fea, seq_size, head_count, name):
    ...
    hist_id_col = tf.cond(tf.constant(seq_size) > cur_batch_max_seq_len,
    ...
    attention_net = self.multi_head_att_net(all_ids, head_count, emb_dim, seq_len, seq_size)
    
    tmp_net = self.add_and_norm(all_ids, attention_net, emb_dim, name='add_and_norm_1')
    feed_forward_net = self.dnn_net(tmp_net, [emb_dim], 'feed_forward_net')
    net = self.add_and_norm(tmp_net, feed_forward_net, emb_dim, name='add_and_norm_2')

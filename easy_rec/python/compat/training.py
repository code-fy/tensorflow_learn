...
class NewCheckpointListenerWithStepLimit(
    basic_session_run_hooks.CheckpointSaverListener):

  def __init__(self, evaluator, eval_throttle_steps):
  ...
  def after_save(self, session, global_step_value):
    ...
    # skip first checkpoint evaluation
    if self._is_first_run:
      self._is_first_run = False
      return

    if self._last_eval_step == 0 or  \
       (global_step_value - self._last_eval_step) >= self._eval_throttle_steps:
      self._evaluator.evaluate_and_export()
      self._last_eval_step = global_step_value
    else:
      logging.info('Skip the current checkpoint eval due to throttle steps'
                   '({} secs).'.format(self._eval_throttle_steps))


class NewCheckpointListener(basic_session_run_hooks.CheckpointSaverListener):
...
class _TrainingExecutor(object):
  ...
  def __init__(self,
    ...
    self._continuous_eval_listener = (continuous_eval_listener or
                                      _ContinuousEvalListener())
  ...
  def run_master(self):
    ...
    if self._eval_spec.throttle_steps is not None:
      saving_listeners = [
          NewCheckpointListenerWithStepLimit(evaluator,
                                             self._eval_spec.throttle_steps)
      ]
    elif self._eval_spec.throttle_secs is not None:
      saving_listeners = [
  ...
  class _Evaluator(object):
    ...
    def evaluate_and_export(self):
      ...
      is_the_final_export = (eval_result.metrics[ops.GraphKeys.GLOBAL_STEP] >=

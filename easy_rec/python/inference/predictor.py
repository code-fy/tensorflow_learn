...
_register_abc_meta = get_register_class_meta(_PREDICTOR_CLASS_MAP,
...
class PredictorInterface(six.with_metaclass(_register_abc_meta, object)):
  ...
  @classmethod
  def check_signature(cls, impl_cls):
    function_names = ['__init__', 'predict']
    return check_class(cls, impl_cls, function_names)
    ...
    with self._graph.as_default():
      with self._session.as_default():
        ...
        if tf.gfile.IsDirectory(model_path):
          ...
          # sort inputs by ids so as to match the order of csv data
          input_info.sort(key=lambda t:t[0])
          self._input_names = [ t[1] for t in input_info ] 

          def _get_dtype(tmp_type):
            input_types_dict = { types_pb2.DT_BOOL   : tf.bool,
                                 types_pb2.DT_DOUBLE : tf.double,
                                 types_pb2.DT_FLOAT  : tf.float32,
                                 types_pb2.DT_INT32  : tf.int32,
                                 types_pb2.DT_INT64  : tf.int64 }
          
          self._input_types = [ _get_dtype(t[2]) for t in input_info ] 
  ...
  def predict(self, input_data_dict, output_names=None):
    ...
    for input_name, tensor in six.iteritems(self._inputs_map):
      ...
      assert tensor_shape[0] is None or (tensor_shape[0] == input_shape[0]),\
        'input %s  batchsize %d is not the same as the exported batch_size %d' %(input_name, input_shape[0], tensor_shape[0])
...
class Predictor(PredictorInterface):
  ...
  def predict_table(self, input_table, output_table,
                    all_cols, all_col_types, selected_cols, reserved_cols,
                    output_cols=None, batch_size=1024, slice_id=0, slice_num=1):
    """
    Args:
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
      ...
      import common_io
      table_writer = common_io.table.TableWriter(output_table, slice_id=slice_id)
      ...
      while True:
        try:
          ...
          sum_t1 += (ts2 - ts1)
          sum_t2 += (ts3 - ts2)
        except tf.python_io.OutOfRangeException as ex:
          break 
  ...
  def predict(self, input_data_dict_list, output_names=None, batch_size=1):
    ...
    assert isinstance(input_data_dict_list[0], dict) or \
           isinstance(input_data_dict_list[0], list), 'input is not a list of dict'
    if batch_size > 0:
      num_batches = int(math.ceil(float(num_example) / batch_size))
    else:
      num_batches = 1
      batch_size = len(input_data_dict_list)
      data_list = input_data_dict_list

    outputs_list = []
    for batch_idx in range(num_batches):
      batch_data_list = input_data_dict_list[batch_idx * batch_size:(batch_idx + 1) *
                                    batch_size]

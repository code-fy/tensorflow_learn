...
import numpy as np
import time
from easy_rec.python.utils.pai_util import is_pai_tf
...
from easy_rec.python.compat import training, exporter
from easy_rec.python.builders import strategy_builder
from easy_rec.python.utils import estimator_utils

load_class.auto_import()

# run local model will not support evaluation during training util tf1.10
if LooseVersion(tf.__version__) >= LooseVersion('1.10.0'):
  EvalSpec = tf.estimator.EvalSpec
  TrainSpec = tf.estimator.TrainSpec
  _train_and_evaluate = tf.estimator.train_and_evaluate
else:
  EvalSpec = training.EvalSpec
  TrainSpec = training.TrainSpec
  _train_and_evaluate = training.train_and_evaluate
...
def _get_input_fn(data_config, feature_configs, data_path=None,
                 export_config=None, run_config=None):
  '''
  Args:
  ...
  input_fn = input_class(data_config, feature_configs, data_path,
                         task_index=task_id, task_num=task_num).\
                 create_input(export_config)
  return input_fn
...
def _create_eval_export_spec(pipeline_config, eval_data):
  ...
  if export_config.exporter_type == 'final':
    ...
    exporters = [LatestExporter(
        name='latest',
        serving_input_receiver_fn=export_input_fn,
        exports_to_keep=1
    )]
  elif export_config.exporter_type == 'best':
    logging.info("will use BestExporter, metric is %s, the bigger the better: %d"
          % (export_config.best_exporter_metric, export_config.metric_bigger))
    def _metric_cmp_fn(best_eval_result, current_eval_result):
      logging.info('metric: best = %s current = %s' % (str(best_eval_result),
                   str(current_eval_result)))
      if export_config.metric_bigger:
        ...
        return best_eval_result[export_config.best_exporter_metric] > \
             current_eval_result[export_config.best_exporter_metric]
    exporters = [ BestExporter(name='best',
                    serving_input_receiver_fn=export_input_fn,
  ...
  eval_spec = EvalSpec(name='val', input_fn=eval_input_fn,
      steps=eval_steps, throttle_secs=10,
      exporters=exporters)
  return eval_spec
...
def _train_and_evaluate_impl(pipeline_config, continue_train=False):
  ...
  Build an EasyRecEstimator, and then train and evaluate the estimator.
  Args:
    pipeline_config: a proto.EasyRecConfig object, specifies
    train_config: model_config, data_config and eval_config
    continue_train: whether to restart train from an existing
                    checkpoint
  Returns:
  ...
  # Tempoary for EMR
  if (not is_pai_tf()) and 'TF_CONFIG' in os.environ:
    ...
    if 'cluster' in tf_config and 'chief' in tf_config['cluster'] and 'ps' in tf_config['cluster'] and ('evaluator' not in tf_config['cluster']):
  ...
  estimator, run_config = _create_estimator(pipeline_config,
  ...
  train_spec = TrainSpec(input_fn=train_input_fn, max_steps=train_steps)
  # create eval spec
...
def evaluate(pipeline_config,
  ...
  '''
  Evaluate for evaluation for eval data in pipeline_config_path,
  the metrics will also be displayed on tensorboard.
  Args:
...
def export(export_dir, pipeline_config_path, checkpoint_path=''):
  ...
  export_config = pipeline_config.export_config
  data_config = pipeline_config.data_config
  serving_input_fn = _get_input_fn(data_config, feature_configs,
                                   None, export_config)
  ...
  if export_config.dump_embedding_shape:
    ...
    embed_files = gfile.Glob(os.path.join(pipeline_config.model_dir, "embeddings",
                                             "*.pb." + ckpt_ver))
    assets_extra = {}

...
class ExitBarrierHook(SessionRunHook):
  """ExitBarrier to make sure that workers wait for the master,
     and exit at the same time. After training finish, master has
     to do evaluation and model export, so master exits a little
     late than workers.
  """
  ...
  def begin(self):
    """Count the number of workers and masters, and
       setup barrier queue
    """
    ...
    with tf.device(tf.DeviceSpec(job='ps', task=0,
      ...
      self._signal_que = tf.FIFOQueue(capacity=self._num_worker,
  ...
  def end(self, session):
    """Only when all workers and master enqueue an
    ...
    while que_size < self._num_worker:
      que_size = session.run(self._que_size)
      time.sleep(5)
      tf.logging.info('waiting for other worker to exit, finished %d, total %d' %(que_size, self._num_worker))
    ...
    def _check_flag_file(is_chief, flag_file):
      logging.info('_check_flag_file: is_chief = %d flag_file=%s' % (is_chief,
                   flag_file))
      if is_chief:
    ...
    register(_check_flag_file, is_chief=self._is_chief,
...
class EmbeddingPartSaver:
  ''' for large embedding serving on eas, large embeddings are partitioned
...
class CheckpointSaverHook(CheckpointSaverHook):
  ...
  def __init__(self,
    ...
    self._embedding_savers = []
    self._other_saver = None
    if separate_save:
      for x in tf.global_variables():
        if 'embedding_weights' not in x.name:
          continue
        name_parts = x.name.split('/')
        if 'embedding_weights:' in x.name or\
          (name_parts[-2] == 'embedding_weights' and 'part_' in name_parts[-1]):
          saver = EmbeddingPartSaver(x) 
          self._embedding_savers.append(saver)
  ...
  def _save(self, session, step):
    ...
    for tmp_saver in self._embedding_savers:
      save_path = os.path.join(save_dir, tmp_saver.name)
      tmp_saver.save(session, save_path, global_step=step)
...
class NumpyCheckpointRestoreHook(SessionRunHook):
  ...
  def begin(self):
      ...
      for var_name, var in six.iteritems(self._name2var_map):
        ...
        if var_name in ckpt_data.keys():
          ...
          if list(var_data.shape) == var_shape:
            ...
            logging.error("variable [%s] shape not match %r vs %r" % (
    ...
    assert len(vars_not_inited.keys()) == 0, 'exist variable shape not inited, restore failed'
...
class IncompatibleShapeRestoreHook(SessionRunHook):
  ...
  def begin(self):
    ...
    for var, var_tmp in six.iteritems(self._incompatible_shape_var_map):
      ...
      logging.info("Assign variable[%s] from shape%s to shape%s" %
      ...
      for var in global_variables:
        ...
        if var_name in ckpt_var2shape_map:
          if restore_status[var_name]:
            logging.warning('variable %s find in more than one checkpoint, skipped %s' % (
                            var_name, ckpt_path))
            continue

# -*- encoding:utf-8 -*-
# Author: mengli.cml@alibaba-inc.com wenmeng.zwm@alibaba-inc.com
# Date: 2018_06-22
...
def _getMethodDeclaration(aMethod):
  try:
...
def check_class(cls, impl_cls, function_names=None):
  """
  Check implemented class is valid according to template class, if function
  signature is not the same, exception will be raised
  Args:
  ...
  ours = _getMethods(cls)
  theirs = _getMethods(impl_cls)
  ...
  for name, method in six.iteritems(ours):
    ...
    if name not in theirs:
      ...
      continue
    ourf = _getMethodDeclaration(method)
    theirf = _getMethodDeclaration(theirs[name])
...
def import_pkg(pkg_info, prefix_to_remove=None):
  ...
  if package_path.startswith('/'):
    ...
    if mod is not None:
      # skip those test files in easy-vision
      if not mod_name.endswith('_test'):
        mod.load_module(pkg_info[1])
    else:
      raise Exception('import module %s failed' % (package_path + mod_name))
  else:
    # almost all files are in easy_vision, we use absolute import
    # to make consistence with the code we write
...
def auto_import(user_path=None):
  ...
  if parent_dir != '': 
    for idx in range(len(pre_defined_dirs)):
        pre_defined_dirs[idx] = (os.path.join(parent_dir, pre_defined_dirs[idx][0]),
...
def register_class(class_map, class_name, cls):
  assert class_name not in class_map or class_map[class_name] == cls, \
    'confilict class %s , %s is already register to be %s' % (cls, class_name,

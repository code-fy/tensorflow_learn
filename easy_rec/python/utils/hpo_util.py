...
def kill_old_proc(tmp_dir, platform='pai'):
  ...
  if platform == 'pai':
    os.system("ps -auxwww | grep easy_rec.python.hpo.pai_hpo | grep python | grep -v grep | awk '{ print $2 }' >> %s" % old_proc_file)
    os.system("ps -auxwww | grep client/experiment_main.py | grep python | grep -v grep | awk '{ print $2 }' >> %s" % old_proc_file)
  else:
    os.system("ps -auxwww | grep easy_rec.python.hpo.emr_hpo  | grep -v grep | awk '{ print $2 }' > %s" % old_proc_file)
    os.system("ps -auxwww | grep client/experiment_main.py | grep python | grep -v grep | awk '{ print $2 }' >> %s " % old_proc_file)
    os.system("ps -auxwww | grep el_submit | grep easy_rec_hpo | grep -v grep | awk '{ print $2 }' >> %s " % old_proc_file)
  proc_arr = []
